class CorrectionsController < ApplicationController
  ...
  def create
    ...
    @correction.matches.push(Match.find(@correction.match_id))
    @correction.matches.push(Match.find(@correction.match_id+1))

    respond_to do |format|
      if @correction.save
        format.html { redirect_to match_path(@correction.match_id), notice: 'Correction was successfully created.' }
  ...
  private
    ...
    def correction_params
      params.require(:correction).permit(:name, :link, :match_id, :typeOfCorrection, :change, :minute)
    end
end
